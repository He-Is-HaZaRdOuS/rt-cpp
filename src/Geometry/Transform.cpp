...
bool Transform::intersect(Ray ray, Hit &hit, f32 tmin, f32 tmax) const {
    ...
    if(m_object->intersect(ray, hit, tmin, tmax)) {
        Vector4 normal0 = m_transform_normal.mult({hit.m_normal.get_x(), hit.m_normal.get_y(), hit.m_normal.get_z()});
